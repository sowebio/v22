...
--      Ada.Text_IO.Put_Line (To_UTF_8 (Host) & Nul);
--      Ada.Text_IO.Put_Line (To_UTF_8 (User) & Nul);
--      Ada.Text_IO.Put_Line (To_UTF_8 (Password) & Nul);
--      Ada.Text_IO.Put_Line (To_UTF_8 (Database) & Nul);
--      Ada.Integer_Text_IO.Put (Port); Ada.Text_IO.New_Line;
--      UXStrings.Text_IO.Put_Line ("---------------");
   ...
   begin
      ...
      if RS.Query_ID /= null then
         --  From MySQL doc : You must also call mysql_free_result() after you
         --  are done with the result set.
         RS.Close;  -- Close calls mysql_free_result()
      end if;
   ...
   overriding function Database_Type
     (C : in out Connection)
      return String
   is
   begin
