   ...
   begin
      if Tio.Is_Open (Handle_Read) then
         Tio.Close (Handle_Read);
          ...
          Msg.Error ("v22.Cfg.close > Should be opened before closing: " & Cfg_File_Read);
      end if;
   end Close;
      ...
      if not Ada.Directories.Exists (To_Latin_1 (Cfg_File_Read)) then
         ...
         if Tio.Is_Open (Handle_Read) then
            Close;
             ...
             Msg.Error ("v22.Cfg.Open > Can't create Cnf file");
         end if;
      end if;
      --  Open in read mode
   ...
   begin
      ...
      if Check_Parameters (Section, Parameter, Value) then
         ...
         if not Cfg_Write (Section, Parameter, Value, Trailing_Comment) then
          ...
          Msg.Error ("v22.Cfg.Set > Invalid character in Section, Parameter or Value");
      end if;
   end Set;
